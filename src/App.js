...
function App() {
  ...
  useEffect(() => {  
    const resultItems = dataTaglines.filter(item =>
      item.title.toLowerCase().includes(input.toLowerCase())
    );
    setTaglines(resultItems);
  }, [input]);

    const getNewTagline = (tagline) => {
        setTaglines(prev => [...prev, tagline]);
    }
  ...
  const itemsCurrentPage = taglines.slice(indexOfFirstItem, indexOfLastItem);
  const paginate = (page) => setCurrentPageNumber(page);
    ...
    <div className="app">
      ...
      <div className="inputs">
        ...
        <div>
          ...
          <input id="search-title" type="text" placeholder="search by title" value={input} onChange={handleSearchInput} />
        </div>
      </div>
      <DataList data={itemsCurrentPage} />
      <Pagination itemsPerPage={itemsPerPage} totalItems={taglines.length} paginate={paginate} />
    </div>
  );
}
